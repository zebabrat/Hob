...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        // Square, not circular — "чёрный квадрат с инициалами" per the
        // mockup. No ring pseudo-element either: flat fill only, matching
        // the app's "no shadows, hairline borders or nothing" rule.
...
function AvatarFallback({
  ...
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn(
        "flex size-full items-center justify-center bg-primary text-sm font-medium text-primary-foreground group-data-[size=sm]/avatar:text-xs",
        className
      )}
      {...props}
    />
