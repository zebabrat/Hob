...
Оба проекта на Vercel, деплой по push в `main`.

| Проект | Переменные |
|---|---|
| backend | `DATABASE_URL` (pooled), опционально `CORS_ORIGIN` |
| frontend | `VITE_API_URL` — домен бэкенда, иначе запросы уйдут на свой же origin |

Бэкенд деплоится как функция: `vercel.json` отключает автодетект фреймворка,
собирает воркспейс бэкенда и переписывает все пути на `api/index.ts`, который
поднимает Fastify без привязки к порту.
